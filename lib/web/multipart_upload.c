/*
 * Multipart upload FFI stub for MBOpenClacky web server (fix-17 skill publish).
 * Provides a synchronous binary-safe HTTP POST/PATCH via WinHTTP (Windows)
 * or libcurl (Linux/macOS). Unlike mbopenclacky_http_post (lib/client), the
 * request body is passed as raw MoonBit Bytes so ZIP payloads containing
 * NUL bytes survive the FFI boundary unchanged.
 *
 * FFI contract:
 *   - method, url, headers: input as MoonBit String (UTF-16, moonbit_string_t)
 *   - headers: formatted as "Key: Value\r\n" pairs (may be empty)
 *   - body: raw request body bytes (moonbit_bytes_t), sent with explicit length
...
static void mp_write_error(moonbit_bytes_t error_buf, const char *msg) {
 ...
 */
static char *mp_string_to_cstr(moonbit_string_t str, int *out_len) {
...
MOONBIT_FFI_EXPORT
int32_t mbopenclacky_web_multipart_upload(
  ...
  int32_t result = -1;
  int method_len = 0, url_len = 0, headers_len = 0;
  char *method = mp_string_to_cstr(method_mbt, &method_len);
  char *url = mp_string_to_cstr(url_mbt, &url_len);
  char *headers_str = mp_string_to_cstr(headers_mbt, &headers_len);
  ...
  mp_write_meta(meta_buf, 0, 0);

  if (!url || !method) {
    mp_write_error(error_buf, "string conversion failed");
    goto win_done;
  }

  if (mp_parse_url(url, w_host, 512, w_path, 4096, &use_https, &port) != 0) {
    mp_write_error(error_buf, "invalid URL format");
    goto win_done;
  }
  ...
  if (!session) {
    mp_write_error(error_buf, "WinHttpOpen failed");
    goto win_done;
  }
  ...
  if (headers_len > 0) {
    ...
    if (whl <= 0) {
      mp_write_error(error_buf, "headers too long for WinHTTP buffer");
      goto win_done;
    }
    w_headers[whl] = 0;
  ...
  BOOL sent = WinHttpSendRequest(
    ...
    (DWORD)body_len, (DWORD)body_len, (DWORD_PTR)0
  );
  ...
  mp_write_meta(meta_buf, (int32_t)status_code, total_read);
  result = 0;
...
win_done:
  if (result != 0) {
    ...
    if (error_buf[0] == 0) {
      ...
      mp_write_error(error_buf, errbuf);
    }
  }
...
static size_t mp_curl_write_cb(char *ptr, size_t size, size_t nmemb, void *userdata) {
  mp_curl_write_ctx *ctx = (mp_curl_write_ctx *)userdata;
...
int32_t mbopenclacky_web_multipart_upload(
  ...
  if (!url || !method) {
    mp_write_error(error_buf, "string conversion failed");
    free(method); free(url); free(headers_str);
    return -1;
  }

  CURL *curl = curl_easy_init();
  if (!curl) {
    mp_write_error(error_buf, "curl_easy_init failed");
    free(method); free(url); free(headers_str);
    return -1;
  }
  ...
  /* POSTFIELDSIZE passes the explicit byte length, so embedded NUL bytes in
   * the ZIP payload are transmitted intact. */
  ...
  mp_curl_write_ctx ctx = { .buf = resp_body_buf, .cap = resp_cap, .len = 0 };
  curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, mp_curl_write_cb);
  ...
  if (res == CURLE_OK) {
    ...
    mp_write_meta(meta_buf, (int32_t)http_code, ctx.len);
  } else {
    resp_body_buf[0] = 0;
    mp_write_meta(meta_buf, 0, 0);
    const char *emsg = (curl_errbuf[0] != '\0') ? curl_errbuf : curl_easy_strerror(res);
    mp_write_error(error_buf, emsg);
  }
