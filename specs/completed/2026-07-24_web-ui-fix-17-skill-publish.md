...
| 2026-07-25 | 实施完成。任务包 0 gate 按已批准处理，平台 base URL 取 orig PRIMARY_HOST（https://www.openclacky.com），可用 MBOPENCLACKY_PLATFORM_URL 覆盖。五步流程全量实现于 lib/web/handlers_publish.mbt（publish_skill），handler 为 handle_my_skill_publish，bridge 保真 403/404/422/500 状态码。multipart 客户端新建 lib/web/multipart_upload.mbt（纯 MoonBit 二进制 body 构建 + 可注入 transport）+ multipart_upload.c（Bytes body FFI，WinHTTP/libcurl 双路径，改造自 lib/client/http_native.c——既有 String body FFI 无法无损传二进制 ZIP，属决策 6 "原生 HttpRequest 扩展"路径）。upload_meta 实现于 lib/brand/config.mbt（load_upload_meta/record_upload，存 ~/.mbopenclacky/skills/upload_meta.json，对齐 orig 文件形状），并已接入 /api/skills 与 /api/creator/skills 的 platform_version/uploaded_at（无记录时仍 null）。偏离：nonce 用 generate_nonce()（12 字节）代替 orig SecureRandom.hex(16)；ZIP 条目排序用自实现 ruby_str_compare（MoonBit String::compare 非字节序）；orig 的 :brand source 分支无对应（本运行时无加密品牌技能）。测试：handlers_publish_wbtest.mbt 19 例（multipart 字节精确断言、ZIP 排除/二进制往返、upload_meta 往返、mock 平台成功/409/500/传输错误/force=PATCH/签名校验、五步各分支、entry 接线）；moon check 0 errors；moon test lib/web 382/382；moon test lib/brand 126/126。 | 按 spec 实施 fix-17 |
