...
| 2026-08-23 | 实施完成并验收：新建 lib/channel/ws_client.mbt（WsClient 薄封装：connect/send_text/recv/close，WsMessage::Text/Closed，基于 @async.websocket Conn，零新增依赖）；discord.mbt 注入 ws_client，handle_gateway_event 改 async 真发送（op 1 心跳/op 7・9 断开/op 10 Hello 后 Identify・Resume/op 11 ack），新增 run_gateway_loop 常驻重连循环（get_gateway_url -> connect -> recv 分发 -> fatal/退避重连），新增 ISO 8601 解析（parse_iso8601_to_ms + days_since_epoch）；lib/channel/moon.pkg 增 moonbitlang/async/websocket。**偏差记录**：(a) WsMessage::Binary 变体删除（Discord 只发文本帧，二进制帧按文本处理）；(b) 心跳定时器未独立起任务，靠 op 1 服务端请求驱动（周期心跳列 backlog）；(c) 钉钉 Stream/WeCom 消费 WsClient 仍为后置依赖未动。验收：discord.mbt 连接层 TODO 清零；moon check 0 errors；moon test lib/channel 412/412；fmt/info 已跑 | 实施完毕，归档 |
| 2026-08-23 | 对抗性审查修订（代码级，验收标准未达标项补齐）：1) 客户端主动心跳缺失（C 级）——原实现仅被动响应服务端 op 1，而 Discord 协议要求客户端按 heartbeat_interval 主动发 op 1（缺失约 60s 被服务端断开陷入无限重连）；重构 run_gateway_loop 为 with_task_group(heartbeat_loop + gateway_connect_loop)：heartbeat_loop 实现协议要求的首跳 jitter、周期心跳、丢 ACK 检测（主动断线由主循环 Resume 重连）；2) op 9 InvalidSession 补 2s 重连延迟（协议要求 1-5s 随机等待）；3) 孤儿层接线失败（C 级，决策 4 未达成）——run_gateway_loop 与 ChannelManager::start 全仓库零调用，send API 恒返 Adapter not running、接收链路断裂；新增 ChannelManager::start_with_gateway(group) 并由 WebServer::start 在 with_task_group 内接线（注册 on_message 日志 handler 并 spawn 网关循环）；4) ws_client.close 置 conn=None（原 close 后 connected 可能仍真）；5) timestamp Int64（见 06 修订）；6) 新增 discord_wbtest.mbt ISO8601 表驱动 4 例（时区偏移/毫秒截断/32 位溢出回归）。验收：moon check 0 errors；moon test lib/channel 416/416 | 实施后对抗性审查发现 2 项 C 级缺陷（心跳缺失、孤儿层） |
