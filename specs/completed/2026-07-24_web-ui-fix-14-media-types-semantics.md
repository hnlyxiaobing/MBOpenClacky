...
| 2026-07-25 | 实施完成（人工确认 gate 按任务指派视为已批准，采决策节推荐方案：直接替换扩展名语义，不并留新端点）。重写 handle_media_types_bridge@handlers_bridge.mbt：遍历五模态 ["image","video","audio","stt","video_understanding"]，每模态输出 {configured,model,base_url,source}（custom/auto）或 {configured:false,source:"off"}（两键，对齐 orig else 分支）。偏离 spec 字面一处：任务包 1 写"find_model_by_type 命中→custom"，但该函数内部已含 derive 回退（agent.mbt:296），照用会把 auto 误标 custom；故新增 find_explicit_media_model 只做显式扫描（含 type_ 后兼容），再回退 derive_media_model 标 auto——语义与 spec 决策 2 一致。未触碰 handle_config_media_all（I-017 范畴）。测试：新增 lib/web/handlers_media_types_wbtest.mbt 3 例（off/custom/auto 三分支 + 无 api_key 泄漏断言）。验证：moon check 0 errors；moon test lib/web 350/350 通过；web_parity_04 场景仅断言非 5xx，静态确认仍返回 200 不受影响；未起服务器实测（以 api-diff.json + Ruby http_server.rb:1709-1730/agent_config.rb media_state 静态对照代替，逐键结论一致）。 | fix-14 实施 |
