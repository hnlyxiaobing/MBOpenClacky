...
| 2026-07-25 | 实施完成：handlers_schedules.mbt 新增任务文件合并（default_tasks_dir/tasks_dir/read_task_file/task_file_to_api_json/manual_task_entries），init_schedule_state 的 tasks_dir 默认值落实为 `~/.mbopenclacky/tasks/`（web 层解析，lib/server 无需改动）；handle_schedules_list 合并 scheduled=false 手动任务条目，且调度条目内容以任务文件为真相源（文件存在时覆盖 messages 缓存）。新增 handlers_schedules_contract_wbtest.mbt 5 个白盒测试（缺目录为空、手动文件条目形状、有调度时跳过、GET 合并、文件内容优先）全部通过；`moon check` 0 errors；`moon test lib/web` 331 通过 327、4 个失败均在 handlers_dirs_wbtest.mbt（并行 dirs 代理的中间态，与本 spec 无关）。偏离：契约断言未放入 handlers_api_contract_wbtest.mbt（共享文件约束），改放 handlers_schedules_contract_wbtest.mbt。 | fix-19 实施 |
