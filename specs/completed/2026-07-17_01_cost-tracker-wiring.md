...
- 修改 `calculate_model_cost` 函数：
  - **字段映射**：`@client.Usage` 的 `input_tokens` → `prompt_tokens`，`output_tokens` → `completion_tokens`，`cache_creation_input_tokens` → `cache_write_tokens`，`cache_read_input_tokens` → `cache_read_tokens`
  - 调用 `@pricing.calculate_cost(...)` 并将 `Result[CostResult, String]` 转换为 `Double?`（Ok → Some(total_cost)，Err → None）
- `moon check` 验证类型正确
...
- [x] `calculate_model_cost` 不再返回 `None`，能根据模型名和 token 用量返回真实成本
- [x] `track_cost` 的 `cost` 字段不再始终为 `0.0`
- [x] `CostSource` 能返回 `Actual`（而非始终 `Estimated`）
- [x] `moon check` 0 errors
- [x] `moon test lib/agent` 通过（224/224）
- [x] `moon test lib/pricing` 通过（39/39，不受影响）

## 风险评估 [必填]

| 风险 | 影响 | 缓解方案 |
|------|------|---------|
| Usage 字段与 pricing 参数命名不一致 | 低 | 已确认映射关系：`input_tokens`→`prompt_tokens`，`output_tokens`→`completion_tokens`，`cache_creation_input_tokens`→`cache_write_tokens`，`cache_read_input_tokens`→`cache_read_tokens` |
...
| 2026-07-17 | 对抗性审查：修正字段映射 | `@client.Usage` 字段名与 pricing 参数名不一致，已确认映射关系 |
