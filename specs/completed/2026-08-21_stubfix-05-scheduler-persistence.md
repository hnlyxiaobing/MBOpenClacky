...
| 2026-08-23 | 实施完成并验收：重写 lib/server/scheduler.mbt——YAML 子集序列化/解析（serialize_schedules_yml/parse_schedules_yml/yml_parse_kv/yml_unquote/yml_quote）、load_from_config（文件缺失空启动，决策 5）、save_config write-through、create_cron_task 写 .md 任务文件 + delete_cron_task 文件删除、tick 时间戳改用 @utils.now_seconds()（复用 stubfix-04 共享时钟）、update_schedule cron 更新用数组索引整条替换（cron 字段非 mut）；lib/server/moon.pkg 增 @fs/@utils。无偏差。验收：moon check 0 errors；moon test lib/server 120/120；fmt/info 已跑 | 实施完毕，归档 |
| 2026-08-23 | 对抗性审查修订（代码级）：1) add/remove/update 原 save_config 错误被 let _ 吞掉仍返回 Ok（写盘失败即重启丢任务，违背错误透传原则）——改为传播 Err；2) update_schedule 原 enabled 先改、cron parse 失败留下半更新内存状态——改为先 parse 后原子替换整条；3) create/delete_cron_task 补 validate_task_name 路径消毒（拒绝斜杠、双点、空名，防目录穿越任务名写穿 tasks_dir）；4) yml_quote/yml_unquote 转义引号与反斜杠（name 含引号原 round-trip 破坏）；5) 新建 scheduler_wbtest.mbt 10 例（持久化 roundtrip/特殊字符/脏数据容忍/删除持久化/无效 cron 不落盘/更新原子性/路径穿越拒绝/重名拒绝/proc 写失败传播/转义 roundtrip）；6) server_wbtest 旧调度器测试改用隔离路径（不再写共享 /tmp/schedules.yml）。验收：moon check 0 errors；moon test lib/server 136/136 | 实施后对抗性审查发现 1 项 C 级假成功 + 4 项 W 级 |
