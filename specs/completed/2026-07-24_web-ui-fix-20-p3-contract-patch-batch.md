...
| 2026-07-25 | 实施完成：I-032 两处 404 错误体补 ok:false（handlers_mcp.mbt handle_mcp_list_server_tools、handlers_skills.mbt handle_skills_get_content）；I-033 handle_store_list/handle_store_installed 补 ok:true；I-034 load_agents_from_assets 按 orig 语义补 avatar（有 avatar.png 则 /agent_avatar/<id>，否则 null，当前资产无头像故为 null）；I-035 handle_backups_list 补 config（orig DEFAULT_CONFIG 九键）/dest_dir/is_wsl（用 @utils.is_wsl() 而非 spec 建议的 is_wsl1()——orig Ruby wsl? 匹配任意 WSL，语义对齐优先）；I-037 web/i18n.js en/zh 补 sessions.untitled。I-038/I-039 调查定位同一根因：Chromium 对未消费响应体的 fetch 不标记完成（200 chunked 挂起→networkidle 超时；204 报 net::ERR_ABORTED），经 TCP 代理抓包 + 隔离实验证实响应字节完整、与 boot 序列无关。小修落地：auth.js _probe 补 r.arrayBuffer()、sessions.js 两处 deleteSession 补 res.text()，登记 web/PATCHES.md P3-001~003；实测 networkidle 693ms 达成、DELETE 正常 finished。根治（服务端 Content-Length 分帧）另立 spec。新增 lib/web/handlers_p3_contract_wbtest.mbt 6 项断言；moon check 0 errors；moon test lib/web 388/388 通过（含新增 6 项）。 | 实施 fix-20 P3 批次 |
