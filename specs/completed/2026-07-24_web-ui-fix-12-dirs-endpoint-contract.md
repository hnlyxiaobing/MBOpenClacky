...
| 2026-07-25 | 实施完成：`handle_dirs_list` 重写为 orig 六键契约（root/path/parent/home/default/entries），仅目录条目（绝对 path+type:"dir"）、小写名称排序、IGNORED 9 项恒过滤、show_hidden 放行隐藏项、空 path 默认 home、`~` 展开、不存在目录祖先回退、`default` 读 config.default_working_dir 否则 `<home>/clacky_workspace`；保留 validate_path 拒绝 `..`；响应路径统一 `\`→`/`。新增包内 helper：`dirs_ignored_entries`、`dirs_fwd_slashes`、`dirs_lex_compare`（core `String::compare` 为 shortlex，与 Ruby 字典序不同）、`dirs_parent`（`@path.Path::dirname` 在 Windows 仅认 `\`，前端发 `/` 路径，故自实现双分隔符 parent）。新建 `handlers_dirs_wbtest.mbt` 6 个测试。验证：`moon check` 0 errors 0 warnings；`moon test lib/web --filter "dirs*"` 9/9 通过；`moon test lib/web` 全量 343 中 339 通过，4 个失败全部位于并行代理在途文件 handlers_billing_contract_wbtest.mbt，与本 spec 无关。 | 按 spec 决策节实施 I-008 对齐 |
