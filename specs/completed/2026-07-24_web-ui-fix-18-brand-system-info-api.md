...
| 2026-07-25 | 实施完成（gate 按批准执行推荐方案）。I-022：handle_brand_config 改为显式 Json::object 输出 orig 13 键（null/false 恒在），弃用 cfg.to_json()，顺带封堵 license_key 明文泄露。I-023：handle_onboard_status 增补 needs_onboard/phase（仅 true 时输出 "key_setup"）/branded，保留 completed/step/skipped。I-024：handle_version_info 增补 current/latest/needs_update/launcher/cli_command 五键并保留原四键；新增进程内 Ref 缓存 fetch_latest_version_cached（TTL 3600s，失败也缓存 None；POST /check、/upgrade 仍实时直连远端）；cli_command 回落 "mbopenclacky"（有意偏差，二进制名不同）。新增 lib/web/handlers_brand_wbtest.mbt 8 个测试（13 键存在性、license_key 不泄露、派生布尔、onboard 两分支、version 五键、launcher 环境变量、branded cli_command、缓存 TTL 命中）。验证：moon check 0 errors；moon test lib/web --filter "*fix18*" 8/8 通过；moon test lib/web 全量无回归。 | 按 spec 决策节推荐方案实施 I-022/I-023/I-024 |
