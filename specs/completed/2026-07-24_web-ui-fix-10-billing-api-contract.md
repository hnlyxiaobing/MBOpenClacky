...
| 2026-07-25 | 实施完成。对照 Ruby `api_billing_summary/daily/records/sessions`（http_server.rb:2855-2919）与 `billing_store.rb` 逐键对齐：summary 改为 orig 12 字段（period/from/to/total_cost/total_tokens/prompt_tokens/completion_tokens/cache_read_tokens/cache_write_tokens/by_model/by_day/record_count，by_day 值为 cost 数值，by_model 条目为 cost/prompt_tokens/completion_tokens/requests，period=all 时 from=null），移除 active/plan/expires_at/total_requests（前端不消费，web/features/billing 已 grep 确认）；daily 键名 daily→days 并补 prompt/completion/cache 细分，补 days 参数（默认 30、上限 90）、全区间零填充；records 补 count/cache_read_tokens/cache_write_tokens/cost_source，timestamp 改 ISO 字符串，limit 默认 100 上限 500；sessions 补 count/total_tokens/token 细分/first_request/last_request/models，按 total_cost 降序，limit 默认 50 上限 200，支持 period 参数。无源字段：session_name 无 SessionManager 数据源，按风险节约定给空串；orig 的 _deleted_ 合并行未实现。已知限制：crescent bridge 的 `event.req.path()` 不含 query string，live 请求走 orig 默认值（period=month/days=30/limit 默认），query 参数解析仅对 path 带查询串的调用生效。新增契约白盒测试 lib/web/handlers_billing_contract_wbtest.mbt（5 个测试）。验证：`moon check` 0 errors；`moon test lib/web --filter "*billing*"` 6/6 通过；`moon test lib/web` 343/343 通过。 | I-009/I-014/I-015 实施 |
