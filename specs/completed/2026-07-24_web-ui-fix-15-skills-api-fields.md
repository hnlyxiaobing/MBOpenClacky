...
| 2026-07-25 | 实施完成：scan_skills 重构为 SkillScanEntry 扫描 + skill_list_entry_json 构建 orig 13 键条目（name/name_zh/description/description_zh/source/always_show/enabled/invalid/warnings/platform_version/uploaded_at/local_modified_at/shadowing_brand，invalid 时附 invalid_reason）；handle_session_skills 去 stub 接 session_skills_json()（合并扫描 + user_invocable 过滤，orig 条目形状含 encrypted/source_type）。无源键按约定给空：platform_version/uploaded_at/local_modified_at=null（无 upload_meta 与文件 mtime API）、warnings=[]、shadowing_brand=false。白盒测试更新 + 新增 handlers_skills_contract_wbtest.mbt；moon check 0 errors，moon test lib/web 363/363。偏离：/api/skills 保留 total 附加键；session 响应保留 session_id 附加键；未复刻 orig 的 agent-profile 过滤与 404 语义。 | 按 spec 决策节实施 fix-15 |
