...
| 2026-07-25 | 实施完成：handle_creator_skills 输出 orig 五键 {ok:true, licensed, cloud_skills:[], local_skills, platform_fetch_error:null}；licensed 接 @brand.BrandConfig::load(brand_config_dir()).user_licensed()；local_skills 复用 fix-15 的 SkillScanEntry 扫描（仅 user 目录，按 name 排序），条目 8 键 {name,description,source,enabled,platform_version:null,uploaded_at:null,local_modified_at:null,shadowing_brand:false}（upload_meta/mtime 无数据源按约定给空）。白盒测试更新（handlers_skills_wbtest.mbt）+ 新增 handlers_skills_contract_wbtest.mbt；moon check 0 errors，moon test lib/web 363/363。cloud_skills 真实数据待 fix-17 发布通路。 | 按 spec 决策节实施 fix-16（fix-15 先行完成后） |
