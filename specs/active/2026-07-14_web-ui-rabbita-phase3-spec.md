...
Phase 0-2.8 已完成 9 个面板的 rabbita 迁移（Brand/Backups/Version/Profile/Share/Trash/ModelTest/Tasks/Browser），但以下关键问题尚未解决：

1. **i18n 功能回归**：全部 9 个已迁移的 rabbita Cell 硬编码英文字符串，不支持中文。原 JS 面板通过 `I18n.t(key)` + `data-i18n` 属性实现双语，迁移后该能力丢失。
2. **15 个面板仍为 legacy JS**（共 ~4,480 行）：包括核心的 Chat/Sessions/WebSocket 集群（~1,255 行，4 文件紧耦合）以及 12 个管理面板。
3. **app.js 仍是协调中枢**（349 行）：提供 API 包装器、通知/模态框、视图切换、WS 事件路由、键盘快捷键。在所有面板迁移完成前无法移除。
4. **#04 spec 仍处于 active**：store/view 拆分方案已被 rabbita 取代，需归档以避免双线推进。

本 Spec 规划 Phase 3（剩余面板迁移）和 Phase 4（全量清理与架构固化）的完整工作。
...
| 声称 | 验证命令 | 结果 | 结论 |
|------|---------|------|------|
| "9 个 rabbita Cell 已存在" | `glob web/mb/main/*_cell.mbt` | 9 文件（brand/backups/version/profile/share/trash/model_test/tasks/browser） | 确认 |
| "9 个 legacy JS 已删除" | `glob web/js/*.js` | 不含 brand/backups/versions/profile/share/trash/model_test/tasks/browser.js | 确认 |
| "bridge.mbt 有 22 个 extern 函数" | `grep 'extern "js"' web/mb/main/bridge.mbt` | 22 匹配 | 确认 |
| "i18n_t 桥接不存在" | `grep 'i18n_t' web/mb/main/bridge.mbt` | 0 匹配（仅有 `i18n_set_locale`） | 确认缺失 |
| "所有 cell 硬编码英文" | `grep -l 'hard-coded in English' web/mb/main/*_cell.mbt` | browser/model_test/share/tasks/trash 均有此注释 | 确认 |
...
| "app.js navModules 含 17 个面板条目" | `grep -c 'btn-' web/js/app.js` (navModules 数组内) | 17 个 navModules 条目（含已删除模块的残留条目） | 确认 |
| "index.html 80 个 data-i18n 属性" | `grep -c 'data-i18n' web/index.html` | 80 | 确认 |
...
| "meeting.js 仅 68 行占位面板" | 读 `web/js/meeting.js` 全文 | 纯 HTML 占位，无 API 调用 | 确认可低风险迁移 |
| "#04 spec 仍在 specs/active/" | `ls specs/active/` | `2026-07-13_04_frontend-feature-architecture.md` 存在 | 确认需归档 |

### 详细分析

#### 已迁移面板现状（Phase 0-2.8 成果）

9 个 rabbita Cell 已在 `web/mb/main/` 落地，全部通过 `cell_with_emit` + bridge.mbt FFI 实现 API 调用。`main.mbt` 中每个 Cell 独立 mount 到对应的 `#xxx-content` 容器，通过 `with_init(emit(Init))` 在页面加载时触发数据拉取。

**关键模式**：
- API 调用通过 bridge.mbt 的 `api_get/api_post/api_put/api_del`（包装 `App.API.*` 全局对象，返回 MoonBit `Json`）
- 通知/模态框通过 `app_notify/app_show_modal/app_hide_modal` FFI
- 仅 browser_cell 使用了 `@sub.every` 声明式订阅（5 秒自动刷新截图）
- 所有 Cell 硬编码英文字符串，未接入 i18n 系统
...
| 分级 | 面板 | 文件 | 行数 | 核心挑战 |
|------|------|------|------|---------|
| 🔴 P0-Critical | i18n 基础设施 | — | — | 9 个已迁移 Cell 的 i18n 回归 |
| 🔴 P1-High | Chat 集群 | chat+sessions+websocket+ws-dispatcher | 1,255 | SSE 流式、Dispatcher 状态机、Markdown FFI |
| 🟡 P2-Medium | Settings | settings.js | 228 | 配置表单、模型选择 |
| 🟡 P2-Medium | Skills | skills.js + skills_enhanced.js | 457 | 代码编辑器、文件管理、保存追踪 |
| 🟡 P3-Standard | MCP | mcp.js | 393 | Server/Tool CRUD |
| 🟡 P3-Standard | Channels | channels.js | 352 | 平台配置、CRUD |
| 🟡 P3-Standard | Schedules | schedules.js | 239 | Cron CRUD |
| 🟡 P3-Standard | Billing | billing.js | 275 | License 管理 |
| 🟡 P3-Standard | Git | git_panel.js | 365 | Git 操作 |
| 🟡 P3-Standard | Creator | creator.js | 321 | 项目脚手架 |
| 🟡 P3-Standard | Onboard | onboard.js | 285 | 引导向导 |
| 🟡 P3-Standard | Media | media.js | 286 | 媒体管理 |
| 🟢 P4-Low | Marketplace | marketplace.js | 184 | 扩展列表 |
| 🟢 P4-Low | Workspace | workspace.js | 206 | 文件浏览 |
| 🟢 P4-Low | Meeting | meeting.js | 68 | 纯占位面板 |

#### app.js 残留职责

app.js（349 行）当前承担以下不可移除的职责：
1. `API` 对象（get/post/put/del）— bridge.mbt 通过 FFI 调用
2. `App.init()` — 初始化所有 legacy JS 模块 + i18n + 通知 + WS 监听
3. `App.showView(name)` — 面板切换（`.view` class toggle）
4. `App.bindEvents()` — 侧边栏、模态框、键盘快捷键、navModules 导航
...
**选择**：将 `specs/active/2026-07-13_04_frontend-feature-architecture.md` 移动到 `specs/completed/`，标注状态为「被 rabbita 迁移取代」。
...
| 文件 | 操作 | 说明 |
|------|------|------|
| `web/mb/main/bridge.mbt` | 修改 | 新增 i18n_t、js_connect_sse、js_render_markdown、app_show_view 等 FFI |
...
| `web/mb/main/settings_cell.mbt` | 新建 | Settings Cell |
| `web/mb/main/skills_cell.mbt` | 新建 | Skills Cell（含代码编辑器） |
| `web/mb/main/mcp_cell.mbt` | 新建 | MCP Cell |
| `web/mb/main/channels_cell.mbt` | 新建 | Channels Cell |
| `web/mb/main/schedules_cell.mbt` | 新建 | Schedules Cell |
| `web/mb/main/billing_cell.mbt` | 新建 | Billing Cell |
| `web/mb/main/git_cell.mbt` | 新建 | Git Panel Cell |
| `web/mb/main/workspace_cell.mbt` | 新建 | Workspace Cell |
| `web/mb/main/creator_cell.mbt` | 新建 | Creator Cell |
| `web/mb/main/onboard_cell.mbt` | 新建 | Onboard Cell |
| `web/mb/main/media_cell.mbt` | 新建 | Media Cell |
| `web/mb/main/marketplace_cell.mbt` | 新建 | Marketplace Cell |
| `web/mb/main/meeting_cell.mbt` | 新建 | Meeting Cell（占位面板） |
| `web/mb/main/*_cell.mbt`（已存在的 9 个） | 修改 | 回填 i18n_t 调用，替换硬编码英文字符串 |
| `web/index.html` | 修改 | 逐步移除 legacy `<script src="js/xxx.js">`，保留 lib/ 和基础设施 |
...
| `web/js/settings.js` | 删除 | 被 settings_cell.mbt 取代 |
| `web/js/skills.js` | 删除 | 被 skills_cell.mbt 取代 |
| `web/js/skills_enhanced.js` | 删除 | 被 skills_cell.mbt 取代 |
| `web/js/mcp.js` | 删除 | 被 mcp_cell.mbt 取代 |
| `web/js/channels.js` | 删除 | 被 channels_cell.mbt 取代 |
| `web/js/schedules.js` | 删除 | 被 schedules_cell.mbt 取代 |
| `web/js/billing.js` | 删除 | 被 billing_cell.mbt 取代 |
| `web/js/git_panel.js` | 删除 | 被 git_cell.mbt 取代 |
| `web/js/workspace.js` | 删除 | 被 workspace_cell.mbt 取代 |
| `web/js/creator.js` | 删除 | 被 creator_cell.mbt 取代 |
| `web/js/onboard.js` | 删除 | 被 onboard_cell.mbt 取代 |
| `web/js/media.js` | 删除 | 被 media_cell.mbt 取代 |
| `web/js/marketplace.js` | 删除 | 被 marketplace_cell.mbt 取代 |
| `web/js/meeting.js` | 删除 | 被 meeting_cell.mbt 取代 |
...
### 任务包 3：Settings + Skills 迁移（预估 2 天）
...
- [ ] 15 个 legacy JS 面板全部迁移为 rabbita Cell
- [ ] 15 个旧 JS 文件全部删除，`grep -r 'require\|import.*js/' web/index.html` 无残留引用（lib/ 除外）
- [ ] 全部 Cell 的可见字符串通过 `i18n_t()` 获取，支持中英文切换
- [ ] Chat 流式渲染、Markdown、工具调用、Subagent/Thinking 卡片功能不回归
- [ ] 会话 CRUD、WS 连接/重连、SSE 流式功能不回归
- [ ] `web/js/` 仅保留 `lib/marked.min.js`、`lib/highlight.min.js`（Phase 4 完成后）
- [ ] `web/index.html` 仅含 `lib/*.js` + `<script type="module" src="mb/index.js">`（Phase 4 完成后）
- [ ] `moon check` 0 errors（`web/mb/`）
- [ ] `warren build main --dist dist` 通过，产物 < 500KB
- [ ] `moon build --target native --release cmd` 0 errors
- [ ] 服务冒烟测试：`/health` 200、`/mb/index.js` 200、所有面板功能正常
- [ ] `#04` spec 归档到 `specs/completed/`
...
**验证**：当前 22 个 extern 函数（200 行）。Phase 3 预计新增 ~15 个（i18n_t、SSE、Markdown、scroll、copy_code、show_view 等），达到 ~37 个。Phase 4 移除 app.js 后可减少 ~10 个（notify/modal/hide/api_*），净增 ~5 个。

**结论**：可接受。bridge.mbt 本质是「legacy interop 层」，会随 legacy JS 的减少而收缩。Phase 4 后预计 ~25 个函数，主要是浏览器原生 API 封装（localStorage、scroll、download、日期格式化）+ JS 库桥接（marked/hljs）。不需要重构。

### Q5: 现有 9 个 Cell 的 i18n 回填是否会引入回归？

**质疑**：逐一替换硬编码字符串为 `i18n_t(key)` 调用，如果 key 不存在于翻译表中，`I18n.t()` 返回 key 本身（而非英文文本），可能导致面板显示翻译 key 而非文本。

**缓解**：回填时对照 `i18n/en.js` 确认每个 key 存在。对于 Cell 新增的文本（不在原 JS 中的），需要在 `i18n/en.js` 和 `i18n/zh.js` 中补充对应 key。任务包 0 包含翻译 key 补全步骤。
