...
> **关联总览**: `specs/draft/2026-08-18_diff-harness-matrix-backlog-overview.md`；diff-harness `docs/FEATURE_MATRIX.md` §1  
...
- [ ] grep 以正则语义匹配（字符类/量词/锚点用例通过），空 pattern 报错
- [ ] grep 默认参数在真实目录树中能收集到文件（探针实证转绿）
- [ ] glob `**/*.ext` 递归匹配正确，mtime 降序，空 pattern/坏 base_path 显式报错
- [ ] file_reader 双轨参数兼容，越界/逆序报错，输出无行号，大文件/目录/空文件/非法 UTF-8 行为对齐
- [ ] 三工具 description 与实现一致
- [ ] `moon check` 0 errors；`moon test lib/tool` 与全量 `moon test` 无回归
