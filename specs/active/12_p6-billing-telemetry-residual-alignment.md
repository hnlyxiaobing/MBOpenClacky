...
> **关联总览**: `specs/active/01_diff-harness-matrix-backlog-overview.md`；diff-harness `docs/FEATURE_MATRIX.md` §11  
> **关联历史 spec**: 边界——错误分类细分（BUG-0086 429/400/402）归 `21_p5-error-classification-alignment.md`；重试间隔/退避/熔断（BUG-0088/0089）归 `15_p5-retry-backoff-circuit-breaker.md`；断流检测管线（BUG-0079）归 `14_p5-stream-truncation-retry-pipeline.md` 与 B4 接线决策；Usage 字段补齐（api_cost 载体）归 B4；本 spec 管矩阵 §11 的**计费/遥测/重试残留**；矩阵旧台账编号已被覆盖，一律使用 `矩阵§11/条目名` 锚点  
