...
| P2 | SQLite 决策 | 0.5 天（A）/ 2-3 天（B） | 需确认方案 |
| P3 | 渠道占位修复 | 2-3 天 | 工作量最大，技术复杂度高 |

### 关键决策

1. **SQLite 方案选择**：建议优先方案 A（改描述），如评审不接受再执行方案 B（实现 SQLite）。需要项目负责人确认。
2. **渠道修复分批执行**：配置加载 → 消息 ID 提取 → 微信 AES，逐步推进。
3. **成本计算直接接线**：定价引擎已完整实现，无需重写。
...
| 子 Spec | 文件名 | 状态 | 工作量 |
|---------|--------|------|--------|
| 成本计算接线 | `2026-07-17_01_cost-tracker-wiring.md` | 讨论中 | 0.5 天 |
| 渠道占位修复 | `2026-07-17_02_channel-placeholder-fixes.md` | 讨论中 | 2-3 天 |
| SQLite 决策 | `2026-07-17_03_sqlite-persistence-decision.md` | 讨论中 | 0.5-3 天 |
| C 头文件提取 | `2026-07-17_04_c-header-files.md` | 讨论中 | 0.5 天 |
| 第三方许可 | `2026-07-17_05_third-party-licenses.md` | 讨论中 | 0.5 天 |
