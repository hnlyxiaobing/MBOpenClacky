...
**待决策**：需要与项目负责人确认选择方案 A 还是方案 B。

**建议**：优先尝试方案 A（改描述），如评审不接受再执行方案 B。

**MoonBit 约束检查**：
- FFI 方案可行：参考 `lib/client/http_native.c` 的 `native-stub` + `link.native` 模式
- SQLite3 是纯 C 库，无复杂依赖，FFI 绑定相对简单
- wasm-gc 后端不支持 FFI，但持久化功能仅在 native 环境使用，不受影响
...
### 方案 A：修改申报描述（预估 0.5 天）
- 修改 README.md 中的持久化描述
- 修改大赛申报材料
- 确认评审方接受

### 方案 B：实现 SQLite（预估 2-3 天）
