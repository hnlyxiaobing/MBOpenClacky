...
> **关联历史 spec**: `specs/draft/2026-08-14_p5-stream-truncation-retry-pipeline.md`（FU-01，重试管道接线）；`specs/completed/2026-07-29_agent-03-empty-response-detection.md`（react 层空响应检测）  
...
- **前置依赖**：FU-01（`2026-08-14_p5-stream-truncation-retry-pipeline.md`）——仅当评审决定做决策 4 的机制搬迁时为硬依赖；按当前建议（不搬迁），本 spec 与 FU-01 可并行，落地顺序不限
