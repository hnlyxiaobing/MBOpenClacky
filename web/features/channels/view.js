...
const ChannelsView = (() => {
  ...
  function _renderCard(platform, data, meta) {
    ...
    card.innerHTML = `
      ...
      <div class="channel-card-body">
        ...
        <div class="channel-test-result" id="test-result-${_esc(platform)}"></div>
      </div>
  ...
  // Probe connectivity through the server's real read-only probe. The result
  // is transient: it reports what the platform answered just now, using the
  // credentials stored in ~/.mbopenclacky/channels.json.
  async function _runTest(platform) {
    const btn = $(`btn-test-${platform}`);
    const out = $(`test-result-${platform}`);
    if (btn) btn.disabled = true;
    if (out) {
      out.textContent = I18n.t("channels.test.running");
      out.className = "channel-test-result";
    }
    try {
      const res = await Channels.test(platform);
      const ok = res.test_result === "success";
      if (out) {
        out.textContent = ok
          ? I18n.t("channels.test.ok", { ms: res.latency_ms ?? 0 })
          : I18n.t("channels.test.failed", { msg: res.error || "unknown" });
        out.className = `channel-test-result ${ok ? "test-ok" : "test-fail"}`;
      }
    } catch (e) {
      if (out) {
        out.textContent = I18n.t("channels.test.failed", { msg: e.message });
        out.className = "channel-test-result test-fail";
      }
    } finally {
  ...
  function _openSetup(platform) {
    ...
    return Channels.runSetup(meta.setupCmd, `Channel Setup — ${meta.name}`);
  }
