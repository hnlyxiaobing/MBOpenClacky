...
| CI 自 2026-08-28 起为红 | GitHub Actions 的 `CI` 与 `Docker` 工作流自 `c4b3fa4b`（2026-08-28，最后一次 success）之后每次运行都失败：失败步骤是 `Run tests`（`moon test --release`），其前的 type check / 警告预算 / 公共 API / 真话台账 / 构建 / 契约探针**全部 success**（步骤级证据取自公开 API）。收尾环境为 Windows：本机无法复现该失败（本平台 `lib/mcp` 会挂起，见上一行），且 `gh` 未登录、job 日志需鉴权（HTTP 403） | **open，范围外**（早于 2026-09 收尾，非本次改动引入）。收尾新增的 `Deterministic capability eval` 与 `Repo stats gate` 已**前移到 `Run tests` 之前**，因此即使该步骤红，这两条新闸门仍会给出信号。待具备 Linux 环境或登录 `gh` 后定位 |
