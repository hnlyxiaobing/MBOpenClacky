...
| 真模型评测波动 | mock LLM 测试无法回答"真模型能否干活" | P2（stretch）：`cmd eval` 真模型评测，结果如实标注波动与成本 |
...
| P2 能力评测分层 | `test/eval/eval_engine.mbt` 仍是单一引擎，无确定性 `tool_harness` 与真模型 `cmd eval 5×3` | stretch（加分项）：本期未实现，范围冻结原则下不进 P0/P1 验收；如需按计划 D10-D12 补做 |
...
当前命中 **159** 条（另有 118 条域术语命中被抑制，抑制规则及理由见 §抑制规则）。
