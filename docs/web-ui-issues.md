...
| ID | 标题 | 要点（期望 orig → 实际 current） | 来源 |
|---|---|---|---|
| I-014 | /api/billing/daily 键名不一致 | `{days:[...]}` → `{daily:[...]}` | API-008 |
| I-015 | /api/billing/records、sessions 缺字段 | 含 count/cache_read_tokens/cache_write_tokens/cost_source → 均缺 | API-009 |
| I-016 | /api/creator/skills 结构不兼容 | `{ok,licensed,cloud_skills,local_skills,...}` → `{skills,total}` | API-011 |
| I-017 | /api/config/media 缺 per-模态详情 | default_provider 五子键 + media.* 六字段 → 缺（39 处 diff） | API-012 |
| I-018 | /api/config/ocr 结构不兼容 | `{default_provider, ocr:{...}}` → 平铺四字段 | API-013 |
| I-019 | /api/config 缺当前模型指针与媒体能力 | current_id/current_index/media_capabilities → 缺 | API-014 |
| I-020 | /api/config/settings 缺 4 个开关 | enable_compression/enable_prompt_caching/memory_update_enabled/proxy_url → 缺 | API-015 |
| I-021 | /api/skills 条目缺 9 个展示字段 | name_zh/description_zh/always_show/warnings 等 → 全缺，中文列表缺名称描述 | API-016 |
| I-022 | /api/brand 缺 13 个品牌键 | product_name/logo_url/activated/... → 仅 `{device_id}` | API-017 |
| I-023 | /api/onboard/status 缺 needs_onboard/branded | 引导门禁字段缺失，**需人工确认**行为影响 | API-018 |
| I-024 | /api/version 缺更新检查字段 | current/latest/needs_update/cli_command → 缺，**需确认**是否有意重设计 | API-019 |
| I-025 | /api/sessions/:id/skills 返回空数组 | orig 同位置 53 个技能 → current `[]`，**需确认**是数据差异还是加载未实现 | API-022 |
| I-026 | PATCH /api/sessions/:id 对未发消息的新会话 404 | 新会话首条消息持久化前只在内存，GET 能读 PATCH 读不到（handlers_session_ext.mbt:514，存储双轨，与 I-002 同源） | UI-006。**Fixed**：创建即持久化后 PATCH/rename 均 200（随 I-002 一并修复验证） |
| I-027 | interrupt 后缺 session_update(idle)/progress(done) | 仅发 `interrupted` 一帧（handlers_ws.mbt:323）→ 前端状态栏卡在 running | WS-003 |
| I-028 | 运行结束状态值为 "completed" 而非 "idle" | 前端 `patch.status==="idle"` 才触发 Tasks/Skills 刷新与 clearProgress（ws-dispatcher.js:268）→ 全部不执行 | WS-004 |
| I-029 | task_finished 投递不到任何正常客户端 | 走了 `broadcast_global` 只发 global_subs（hub.mbt:140），按会话订阅的前端永远收不到 → 完成提示音失效 | WS-005 |
| I-030 | 会话摘要字段名与前端期望不符 | `latest_latency_ms` vs 前端读 `latest_latency`（sessions.js:3301），且缺 error_code/top_up_url/raw_message/model_id/card_model/channel_info → 延迟信号永不显示；与 I-012 同源 | WS-007。**Partially Fixed**：`latest_latency` 已改为对象 `{ttft_ms, duration_ms}`（随 I-011 spec 修复）；error_code/top_up_url/raw_message/model_id/card_model/channel_info 六个字段仍缺，留待 fix-04（API 响应包装）或后续 spec |

## P3（8 条）

| ID | 标题 | 要点 | 来源 |
|---|---|---|---|
| I-032 | 部分 404 错误体缺 `ok:false` | /api/mcp/:name/tools、/api/skills/:name/content | API-020 |
| I-033 | /api/store/* 缺 `ok` 字段 | 前端若以 resp.ok 判成败会误判 | API-021 |
| I-034 | /api/agents 条目缺 avatar | — | API-023 |
| I-035 | /api/backup/status 缺 config/dest_dir/is_wsl | — | API-024 |
| I-036 | /api/browser/status 缺 chrome_version | — | API-025 |
| I-037 | i18n key `sessions.untitled` 缺失显示原始 key | 上游也缺该 key，但 current 因 I-002/I-011 实际触发；`I18n.t` 返回 key 使 `\|\| "Untitled"` 兜底失效 | UI-004 |
| I-038 | 删除会话出现一次 DELETE net::ERR_ABORTED | 疑似前端双发 DELETE，最终生效；需人工确认 | UI-007 |
| I-039 | 首页 networkidle 15s 未达成（orig 2.8s vs current 16.8s） | 存在长挂请求，不影响交互，需定位具体请求 | UI-008 |
| I-040 | token_usage 发两次（第二帧 delta=0） | 前端 appendTokenUsage 每帧建 DOM → 界面两行 token 用量 | WS-006 |

## 已核实为 version-skew 或有意差异（不计入）

- 主题切换入口位置、头部 `#reload-header`、新会话 Advanced options、Extensions Brand 过滤 tab、accent 色板：orig v1.5.0 新增，current 前端基线 v1.4.0（见 G-001）。
- `sib-id` 文案（"Session file (xxxxxxxx)" vs 裸 id）：v1.5.0 文案改进。
