...
| 源文件 (非测试) | 174 个 | **194 个** |
| 测试文件 | 39 个 | **43 个** |
| 源代码行数 | ~27,000+ 行 | **~34,400 行** |
| 测试用例 | 969 个 | **1,203 个** |
| `moon check` warnings | 484 | **556** |
| `moon check` errors | 0 | **0** |
| Phase 覆盖 | 0-17 | **0-18** |

### 11.2 文档校准范围

- `CLAUDE.md` — 修复文档内容重复 bug（新增内容 158 行后接旧版内容 109 行），全面重写为单版本
...
## 12. Phase 21 更新：业务功能差距系统性补齐期间的警告治理

> 日期: 2026-06-26
> 范围: Phase 21 业务功能差距系统性补齐期间的编译警告治理进展

### 12.1 警告变化

| 指标 | Phase 20 结束时 | Phase 21 结束时 | 变化 |
|------|-----------------|-----------------|------|
| `moon check` errors | 0 | **0** | 持平 |
| `moon check` warnings | 672 | **276** | **-396 (−58.9%)** |
| 源代码行数 | ~34,400 | **~43,157** | +8,757 (+25.5%) |
| 源文件数 | 194 | **235** | +41 |
