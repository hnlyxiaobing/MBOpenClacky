...
- 严重度：P1
- 范围：`web/` 全部前端资产
- 现状：当前项目前端是上游 **v1.4.0** 的托管 fork（2026-07-22 导入，见 `web/UPSTREAM_SYNC.md`），原项目已是 **v1.5.0**（`openclacky/lib/clacky/version.rb:4`）。
- 影响：v1.5.0 前端的新面板/新交互在当前项目整体缺失；对比测试时部分"差距"实际源于版本差而非复刻遗漏，升级基线后需重新核对本清单。
- 建议：按 `web/UPSTREAM_SYNC.md` 的 rsync 流程做一次 v1.5.0 同步，再重跑对比。
- 状态：Open
...
- 当前项目行为：默认监听 `0.0.0.0:7071`，未设 `MBOPENCLACKY_WEB_API_KEY` 时**完全无认证**（`cmd/main.mbt:434`）；且 loopback 免认证的客户端 IP 取自可伪造的 `X-Forwarded-For`/`X-Real-IP`（`lib/web/server.mbt:144-156` 注释自认"只是开发便利而非安全边界"），公网暴露时伪造该头即可绕过认证。
- 建议：绑定非回环地址且未设 key 时拒绝启动或强制仅监听 127.0.0.1；loopback 判定改用真实对端 IP。
- 状态：Open
